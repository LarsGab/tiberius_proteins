#!/usr/bin/env bash
#SBATCH --job-name=ins_miniprot_gffcmp
#SBATCH --array=0-9
#SBATCH --cpus-per-task=2
#SBATCH --mem=16G
#SBATCH --time=01:00:00
#SBATCH --output=logs/ins_07_miniprot_gffcmp_%A_%a.log
...
echo "[ins_gffcmp] Species=$SPECIES  Level=$LEVEL_LABEL"
...
run_gffcmp_and_extract() {
    ...
    echo "[ins_gffcmp] Comparing Tiberius vs miniprothint ($label) ..."
    gffcompare \
        -e 3 \
        -T \
        -r "$ref" \
    ...
    echo "[ins_gffcmp] Done ($label): $out_tsv"
}
